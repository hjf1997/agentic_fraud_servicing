"""Scenario: Disputed DoorDash Dash Pass — no matching historical amount.

A cardmember disputes a $96 DoorDash - Dash Pass charge from a few days ago.
Transaction records show prior DoorDash transactions at various amounts (food
delivery orders), but none at $96 and none labeled "Dash Pass".
...
def _create_case(gateway: ToolGateway, case_id: str, call_id: str) -> Case:
    """Create the initial Case for the DoorDash Dash Pass dispute scenario."""
    ctx = AuthContext(agent_id="simulation", case_id=case_id, permissions={"write"})
    case = Case(
        case_id=case_id,
        call_id=call_id,
...
def build_scenario() -> Scenario:
    """Build and return the DoorDash Dash Pass dispute scenario (no matching history)."""
    return Scenario(
        name="doordash_dashpass",
        title="Disputed DoorDash - Dash Pass Charge ($96 CNP, no matching history)",
        description=(
            "Cardmember disputes a $96 DoorDash - Dash Pass charge from days ago.\n"
            "Transaction records show prior DoorDash orders at various amounts\n"
            "(food delivery), but none at $96 and none labeled 'Dash Pass'."
        ),
        case_id="case-sim-ddp-001",
        call_id="call-sim-ddp-001",
...
register_scenario("doordash_dashpass", build_scenario)
